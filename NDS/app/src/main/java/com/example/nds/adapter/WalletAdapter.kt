...
class WalletAdapter: ListAdapter<CoinTrans, WalletAdapter.CoinViewHolder>(diffUtil) {
    inner class CoinViewHolder(private val binding: CoinBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(coinMdel: CoinTrans) {
            binding.transDateTextView.text = coinMdel.transDate
            binding.transContentTextView.text = coinMdel.transContent
            binding.transPriceTextView.text = coinMdel.transPrice.toString()
            binding.transRemainTextView.text = coinMdel.transRemain.toString()
            binding.transIoTextView.text = coinMdel.transIo
